...
# Example categorical
Transaction_Type = st.selectbox("Transaction Type", ["Bank Transfer", "Online", "POS","ATM Withdrawal"])
device_type = st.selectbox("Device Type", ["Mobile", "Tablet", "Desktop"])
Location = st.selectbox("Location", ["Mumbai", "New York", "Sydney", "Tokyo","London"])
Merchant_Category = st.selectbox("Merchant Category", ["Electronics", "Groceries", "Restaurants", "Travel","Clothing"])
Card_Type = st.selectbox("Card Type", ["Discover", "Mastercard", "Visa","Amex "])
Authentication_Method = st.selectbox("Authentication Method", ["OTP", "PIN", "Password","Biometric"])
